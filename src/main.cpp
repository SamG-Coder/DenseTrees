...
struct App {
    ...
    float sunAzimuth = .55f;
    float windStrength = .72f;
    bool dragging = false;
    POINT last{};
    ...
    bool handleSceneKey(HWND window,WPARAM key) {
        ...
        if(key==VK_LEFT){sunAzimuth-=.12f;return true;}
        if(key==VK_RIGHT){sunAzimuth+=.12f;return true;}
        if(key=='W'){windStrength=windStrength>.01f?0.0f:.72f;return true;}
        return false;
    }
    ...
    bool createDebugPanelWindow(HWND owner,HINSTANCE instance) {
        ...
        RECT bounds{0,0,scaled(344),scaled(350)};
        AdjustWindowRectEx(&bounds,style,FALSE,extendedStyle);
        debugPanel = CreateWindowExW(extendedStyle,debugWindowClass,L"Grass and ground - F1",
...
LRESULT CALLBACK debugPanelProc(HWND window,UINT message,WPARAM wParam,LPARAM lParam) {
    ...
    case WM_HSCROLL:
        ...
        return 0;
    case WM_COMMAND:
        if(owner&&LOWORD(wParam)==resetButtonId){owner->resetDebugSettings();return 0;}
        break;
    case WM_CLOSE:
...
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,PWSTR,int show) {
    ...
    while(running) {
        ...
        if(running) {
            ...
            app->renderer.render(app->yaw,app->pitch,app->distance,app->sunAzimuth,
                                 app->windStrength,app->debugSettings);
